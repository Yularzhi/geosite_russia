...
Все источники очищены от доменов `category-ru` (полный Direct-набор, включая `sources/category-ru-direct.txt`) — ни один домен не попадает одновременно в Direct и Proxy. Дополнительно список Loyalsoldier очищен от российских суффиксов и зон `.ru/.su/.рф`; списки antifilter и ручных дополнений сохраняются как есть — заблокированные в РФ ресурсы живут в том числе на российских доменах.
...
Код сборки — MIT (см. [LICENSE](LICENSE)).  
Репозиторий агрегирует данные из сторонних источников — смотри лицензии в оригинальных проектах.
